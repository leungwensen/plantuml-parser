...
gulp.task(
  'commonjs2es6',
  () => gulp.src(path.resolve(__dirname, '../src/**/*.js')) // FIXME modify this to specify target
    .pipe(plumber())
    .pipe(toES6())
    .on('error', (err) => {
      gutil.log(gutil.colors.red(err.message));
    })
    .pipe(gulp.dest(path.resolve(__dirname, '../lib/'))) // FIXME modify this to specify target
);
